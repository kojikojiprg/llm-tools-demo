...
from common.file_utils import read_csv
from tools.anomaly_detection.detector import detect_anomalies, build_anomaly_summary
...
def _plot(df, date_col, value_col):
    ...
    fig.add_trace(go.Scatter(
        ...
        y=pd.concat([df["rolling_mean"] + 2*df["rolling_std"],
                     (df["rolling_mean"] - 2*df["rolling_std"])[::-1]]),
        fill="toself", fillcolor="rgba(37,99,235,0.08)",
        line=dict(color="rgba(0,0,0,0)"), name="±2σ範囲"
    ))
    fig.add_trace(go.Scatter(
        x=df[date_col], y=df["rolling_mean"],
        line=dict(color="#2563eb", width=1.5, dash="dot"), name="移動平均"
    ))
    fig.add_trace(go.Scatter(
        x=normal[date_col], y=normal[value_col],
        mode="lines+markers", line=dict(color="#64748b", width=1.5),
        marker=dict(size=4), name="通常"
    ))
    fig.add_trace(go.Scatter(
        x=anomaly[date_col], y=anomaly[value_col],
        mode="markers",
    ...
    fig.update_layout(
        title="時系列データと異常検知結果",
        xaxis_title="日付", yaxis_title=value_col,
        hovermode="x unified",
...
def render():
    ...
    with st.expander("📥 サンプルCSVを生成して試す"):
        if st.button("サンプルデータを生成"):
            ...
            sales[20] += 60; sales[45] -= 50; sales[70] += 40
            sample = pd.DataFrame({"date": dates.strftime("%Y-%m-%d"), "sales": sales.round(0)})
            st.download_button("📥 sample.csv", sample.to_csv(index=False).encode("utf-8-sig"), "sample.csv", "text/csv")

    uploaded = st.file_uploader("CSVファイルをアップロード（日付列＋数値列）", type=["csv"])
    if not uploaded:
        return
    ...
    if count > 0:
        ...
        adf = df[df["is_anomaly"]][[date_col, value_col, "rolling_mean", "pct_change", "z_score"]].copy()
        adf.columns = ["日付", "値", "移動平均", "変化率(%)", "Zスコア"]
        st.dataframe(adf.style.format({"値": "{:.1f}", "移動平均": "{:.1f}", "変化率(%)": "{:+.1f}", "Zスコア": "{:.2f}"}), use_container_width=True)
